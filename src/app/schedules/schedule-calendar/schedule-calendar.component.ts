...
import { ClientScheduleAppointmentModel, SaveScheduleModel, ScheduleAppointmentMonthModel, SelectClientModel } from '../schedules.models';
...
@Component({
  ...
  imports: [CommonModule, MatDatepickerModule, FormsModule,
    ...
    MatPaginatorModule, MatTooltipModule, MatTimepickerModule,
...
export class ScheduleCalendarComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  requestDelete(schedule: ClientScheduleAppointmentModel){
    this.subscription = this.dialogManagerService.showYesNoDialog(YesNoDialogComponent,{title: 'Excluir agendamento', content:'Confirmar cancelamento do agendamento'})
    .subscribe(result => {
      if(result){
